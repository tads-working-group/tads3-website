...
[*Prev:* Scope](scope.htm)     [*Next:* Action Reference](actionref.htm)
   
...
1.  Enclose the complete definition between \#ifdef \_\_DEBUG and
    \#endif preprocessor commands to ensure that your custom debugging
    commands are included only when you compiple for debugging.
...
definitions with \#ifdef \_DEBUG and \#endif so that they don't cause
compilation errors in a release build; for example:
...
When your beta-testers test your game, it is often helpful to get them
to record a transcript of it (using the SCRIPT command), in the course
of which they can type comments such as:
...
[Table of Contents](toc.htm) \| [Actions](action.htm) \> Debugging
Commands  
[*Prev:* Scope](scope.htm)     [*Next:* Action Reference](actionref.htm)
   
