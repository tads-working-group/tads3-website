...
[Table of Contents](toc.htm) \| [Final Moves](final.htm) \> The Web UI  
[*Prev:* Lists and Listers](lister.htm)     [*Next:*
...
- If you follow the [Adv3LiteWeb usage tips](#Adv3WebUsageTips) we'll
  ...
  - Alternatively, if you have extensive code for the two modes, it can
    ...
    build. Adv3Lite itself uses this approach - it has a file called
    console.t that's only included in the traditional Adv3Lite library
    source list, and another called browser.t that's only included in
    the Adv3LiteWeb source list.
...
Instead of the interpreter built-ins, always use the inputManager
equivalents:
...
getInputLine() and getKey() (or their macro equivalents) this parameter
can be omitted altogether; you can simply display a prompt string before
calling the method.

For the most common cases you can use the following macros:

- **more** expands to inputManager.pauseForMore()
- **input(x)** expands to inputManager.getInputLine(x)
- **waitKey(x)** expands to inputManager.getKey(x)
...
Web UI. However, note that the Adv3Lite library only calls
