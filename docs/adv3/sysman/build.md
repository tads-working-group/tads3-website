...
[Table of Contents](toc.htm) \| [The Tools](tools.htm) \> Compiling and
Linking  
[*Prev:* The Tools](tools.htm)     [*Next:* Universal
Paths](univpath.htm)    
...
stutter (double) it: for my "file".txt, write "my ""file"".txt".
...
- *filename*=*alias* - this adds the named file as a multi-media
  ...
  That bundles the local file called graphics\title.jpg, but names the
  resource pics/main.jpg. This overrides the default resource name
  (graphics/title.jpg) that would have been used if the alias hadn't
  been specified.
...
- resource: specifies a multimedia resource file (such as a .jpg image
  ...
  resources/sound.mp3. (As always with resource files, the local Windows
  path separator character "\\ is automatically translated to the
  universal URL-style "/" separator.)
