...
-   If the local\'s value is of type object, call the object\'s virtual
    (metaclass) \"index value\" method, and push the resulting value
    onto the stack. If the metaclass doesn\'t define the method, try
    [operator overloading](#opov) with the imported property symbol
    \"operator \[\]\".
-   If the local is of any other type, throw an error
    (CANNOT_INDEX_TYPE).
...
to achieve the same effect. []{#opc_idxint8}
...
the integer value given by *index_val*. []{#opc_new1}
...
the new object\'s superclass, and invokes the new object\'s
\"construct\" method, passing the remaining arguments as parameters to
the method.
...
will be the \"self\" object for the purposes of the constructor) and
hence will not be deleted during garbage collection. In any other case,
the new object will be stored in register R0 by the metaclass
constructor and will thus be reachable.
...
operands exceeds the 8-bit capacity of NEW1. []{#opc_trnew1}
...
This instruction performs the same operation as [NEW1](#opc_new1), but
the object created is [transient](model.htm#transient). []{#opc_trnew2}
...
the object created is [transient](model.htm#transient). []{#opc_inclcl}
...
Stack: *same as* [NEW1](#opc_new1) []{#opc_declcl}
...
doing so would be faster. []{#opc_addilcl1}
...
local variable at index *local_number*. []{#opc_addilcl4}
...
the local variable at index *local_number*, using the same algorithm
that the [ADD](#opc_add) instruction uses. Assign the result to the
local variable at index *local_number*. []{#opc_addtolcl}
...
Sets the given local to zero. []{#opc_zerolcl2}
...
Sets the given local to zero. []{#opc_nillcl1}
...
Sets the given local to nil. []{#opc_nillcl2}
...
Sets the given local to nil. []{#opc_onelcl1}
...
Sets the given local to the numeric value 1. []{#opc_onelcl2}
...
Sets the given local to the numeric value 1. []{#opc_setlcl1}
...
value in the local variable at index *local_number*. []{#opc_setlcl2}
...
Pop the top element of the stack, calling the value *val*. Store the
value in the local variable at index *local_number*. []{#opc_setarg1}
...
Pop the top element of the stack, calling the value *val*. Store the
value in the parameter variable at index *arg_number*. []{#opc_setarg2}
...
Pop the top element of the stack, calling the value *val*. Store the
value in the parameter variable at index *arg_number*. []{#opc_setind}
...
-   If *container_val* is of type list constant, check the type of
    *index_val*; if the value is not numeric, throw an error
    ...
    value to ensure that it\'s in the range 1 to the number of elements
    in the constant list, inclusive; throw an error (INDEX_OUT_OF_RANGE)
    if not. Create a new list object in which the element at index
    *index_val* (1 refers to the first element) from the original list
    is replaced by *newval*, and the other elements have the same values
    as their corresponding elements in the original list. Push the
    resulting object.
-   If *container_val* is of type object, invoke the object\'s virtual
    ...
    *newval* as parameters. Push the result. If the metaclass doesn\'t
    define a set-index method, try [operator overloading](#opov) with
    the imported property symbol \"operator \[\]=\".
...
PUSHINT8 5 instruction would accomplish this. []{#opc_setprop}
...
as with SETPROP, but use the \"self\" object rather than popping an
object from the stack. []{#opc_objsetprop}
...
than popping an object from the stack. []{#opc_setdblcl}
...
stack_level means. []{#opc_setdbarg}
...
local variable in the current frame to the given value. []{#opc_loadctx}
...
Create a \"stored method context\" and push its value onto the stack.
The method context consists of the current \"self\" object, the current
\"original target object\" value, the current \"defining object\" value,
and the current \"target property\" value.
...
with the method context values. []{#opc_setlcl1r0}
...
variable at index *local_number*. []{#opc_setindlcl1i8}
...
local variable when the index is a small integer constant. []{#opc_bp}
