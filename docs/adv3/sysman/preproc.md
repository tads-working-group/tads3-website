...
Preprocessor  
[*Prev:* Source File Character Sets](charmap.htm)     [*Next:*
...
The compiler still accepts \#pragma newline_spacing(on) as equivalent,
so existing code will work without any changes.)
...
it, with each line break turning into a \n (newline) character, and all
subsequent spacing left intact. This can be useful if you're
constructing multi-line strings for special purposes, such as storing in
external files.
