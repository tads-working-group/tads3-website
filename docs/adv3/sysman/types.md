...
[*Prev:* The Preprocessor](preproc.htm)     [*Next:* String
Literals](strlit.htm)    
...
nil is a special value that means "false" in contexts where a condition
is being tested (such as an if and for statements, or the && or \|\|
operators). nil is also used to represent "empty" values; in particular,
it's frequently used where an object reference would otherwise be used
to mean "no object."
...
- A decimal integer constant is a non-zero digit (1 to 9) followed by
  ...
  15 25 2147483647
- A hexadecimal integer constant is written as 0x followed by one or
  more hex digits (0 to 9, a to f, or A to F): 0x1A 0xFFFF 0x10000000
...
When integers are used with logical operators, with ? :, or as condition
expressions in if, for, while, do...while statements, 0 is treated as
"false" and any non-zero value is treated as "true."
...
value, so it's guaranteed that a != b for any two distinct enumerator
names a and b in a single program.
...
This doesn't evaluate the property getOwner on any object; instead, it
simply stores the ID of getOwner in the local variable a. This variable
can be used to evaluate the property on a particular object at a later
...
You can also get a pointer to a function using the & operator, although
it's not required:
...
a pointer to a built-in function. In this case the & operator is
mandatory, because merely writing the name of a built-in function has
the effect of calling it, even without arguments. The & operator tells
...
certain operators; the + operator can be used to create a new list by
appending elements to an existing list's contents, for example.
...
with certain operators; the + operator can be used to create a new
string by concatenating the contents of a given string with another
...
using the . operator to combine the object's reference with the property
ID of the property or method.
...
The E (which can be upper- or lower-case) can be used for "scientific
notation," to specify a power of ten by which to multiply the part
before the E. For example, 1.25e9 means 1.25×10⁹, or 1.25 billion, and
7.20e-3 means 7.20×10⁻³, or 0.00720.
...
non-zero digit. For example, 0.000010 has a precision of 2, since the
first significant figure is the 1, and the trailing zero counts as
another.
...
(specifically the grammar object declaration statement) that make it
easier to create GrammarProd objects.
...
contents of a Collection. The foreach statement uses an Iterator
internally to carry out its iteration, but you can use these objects
explicitly as well.
