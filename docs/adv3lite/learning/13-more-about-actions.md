...
dobj}
     or `{1}
    . The numbers in curly braces refer to the corresponding parameter in the (optional) list of params that follows, so that, for
...
obj}
    ;
 the theName of obj, marked as the subject of the next verb to follow.
...
obj}
    ;
 the aName of obj, marked as the subject of the next verb to follow.

●

    {the obj}
    ;
 theName of obj, regarded as the object of a verb.
...
obj}
    ;
 the possessive form of obj (e.g. "Brian's", or "my", or the "baker's")
...
obj}
    ;
 the reflexive form of obj (e.g. "herself", "himself", or "yourself")

●

    {i}
    ;
 the current actor, in the subjective case (e.g. "I", "you", "Bob", or "the baker")

●

    {me}
    ;
 the current actor, in the objective case (e.g. "me", "you", "Bob", or "the baker")

●

    {my}
    ;
 the possessive of the current actor (e.g. "my", "your", "Bob's")
...
obj}
    ;
 'that' or 'those' (depending on whether obj is singular or plural), regarded as the object of verb.
...
obj}
    ;
 'that' or 'those' as the subject of the following verb.
...
●
    ...
    {am}
     or `{are}
    ;
 the verb 'to be' in agreement with the most recent subject;
 in the absence of such a subject, the subject is assumed to be in the third person singular.

●

    {come}
    ;
 the verb 'to come' in agreement with the most recent subject, and so on for about two hundred other irregular verbs.

●

    {dummy}
    ;
 a dummy subject marker that displays nothing, but makes any verbs that follow agree with a third-person singular subject.

●

    {plural}
    ;
 a dummy subject marker that displays nothing, but makes any verbs
...
●
    ...
    love{s/d}
    ;
 the verb 'to love' in agreement with the most recent subject, and so on for all the other regular verbs that follow this pattern.

●

    frown{s/ed}
    ;
 the verb 'to frown' in agreement with the most recent subject, and so on for all the other regular verbs that follow this pattern.

●

    splash{es/ed}
    ;
 the verb 'to splash' in agreement with the most recent subject, and so on for all the other regular verbs that follow this pattern.

●

    cr{ies/ied}
    ;
 the verb 'to cry' in agreement with the most recent subject, and so on for all the other regular verbs that follow this pattern.

●

    stop{s/?ed}
    ;
 the verb 'to stop' in agreement with the most recent subject, and so on for all the other regular verbs that double their final consonant in the past tense.
...
obj` is the message name, and it can be used if we want to change the message to something else, either because we're translating the library wholesale into another language, or because we want to customize the library's messages. To customize library messages we need to define a `CustomMessages` object. This contains a list of `Msg()** macros which in turn give the message name followed by the message string we want to replace it with, for example:
    ...
    CustomMessages
       messages = [       Msg(cannot see obj, '{The subj dobj}
 {isn\'t}
 anywhere to beseen. ')
...
. '),        Msg(fixed in place, 'Idiot;
 any fool can see {the subj dobj}
{is}
 firmly
...
Note that in such cases we can't change the parameters that place-holders like `{1}
     refer to;
 they continue to retain the same meaning they had in the original `BMsg()` and `DMsg()` definitions. We can, however, use the place-holder mechanism to construct strings outside the BMsg()/DMsg() mechanisms by using the `dmsg()` and `bmsg()` functions;
 e.g.:
...
'Spain', 
 ...
 bend {1}
. ', theName)
Finally, there's a couple of properties of `CustomMessages` we can use to determine which `CustomMessages` object to use. The `priority` property determines which CustomMessages object takes precedence if more than one overrides the same message;
 the higher the priority the higher the precedence (to override library messages use a `priority` of 200 or above). The `active` property determines whether the CustomMessages object will be used at all;
 you could switch this between true and nil, for example, to change the tone of your game between different narrators.
...
In this case if we enter the command `take ball and bat`, the `exit `statement will prevent the ball being taken, but not the bat, and we'll still see one of the atmospheric messages like 'A gust of wind blew.' If we changed `exit` to `abort` then we'd just see the refusal to do anything with the ball;
 there'd be no attempt to take the bat and no atmospheric message. If we had entered the command `take ball and bat;
 throw bat;
 go south `however, then the commands `throw bat` and `go south` would be executed even with `abort`, but we'd only see one atmospheric message instead of the two we would have seen had we used `exit` rather than `abort`. In other words, each separate command entered on the command line is treated as separate;
...
Both `exit` and `abort` are in fact macros that throw Exceptions. This introduces a feature of TADS 3 programming we haven't encountered before;
 we'd better explain it now.
...
man;
 ...
 Bob can hear you                perfectly well. ";
...
If we want the player's *location  *to intervene, however, we should use its `roomBeforeAction() `method;
 e.g.:
    ...
    lowCave: Room 'Low Cave'
       ...
       roomBeforeAction()   {
        ...
        "You\'d better not jump here;
 you\'d bump your head onthe           low ceiling. ");
...
;
 man;
 him'   afterTravel(traveler, connector)
       {      if(traveler == me)
...
me` or with `gActor.isPlayerChar()**;
 for example:
    ...
    largeBox: OpenableContainer 'large box'    dobjFor(Take)
        ...
        "You're too much of a weakling to pick it up;
 you'll              have to persuade someone else to carry it for you. ";
...
Note that all of these are ways of making Bob act 'spontaneously' (under program control);
 they are not ways in which the *player* or *player character* gives orders to Bob, they are ways in which the *game author* can make Bob do things.
