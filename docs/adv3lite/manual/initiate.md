...
[Table of Contents](toc.htm) \| [Actors](actor.htm) \> NPC-Initiated
Conversation  
[*Prev:* String Tags and Object Tags](tags.htm)     [*Next:* Changing
the Player Character](changepc.htm)    
...
InitiateTopics are triggered by calling initiateTopic(obj) on the actor.
If there's one or more InitiateTopics defined on the actor (or the
actor's current ActorState) that have *obj* as one of the objects on
their matchObj property, then the InitiateTopic that's the best match
(i.e. the one with the highest matchScore that has isActive = true) is
selected and its topicResponse is displayed (much as would be the case
for any other kind of ActorTopicEntry).
...
define it as yourNewCondition && inherited), a ConvAgendaItem will be
triggered when no conversation has taken place on that turn and the NPC
is in a position to talk to the player character but isn't currently at
a [conversation node](convnode.htm). A ConvAgendaItem can be used:
...
The advantage of using actorSay() here instead of just using a
...
of and reasons for using actorSay() in this kind of situation see the
