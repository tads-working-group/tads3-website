...
[Table of Contents](toc.htm) \| [Actors](actor.htm) \> Topic Groups  
[*Prev:* Special Topics](specialtopic.htm)     [*Next:* Conversation
Nodes](convnode.htm)    
...
\[convKeys\] ?, so that both the scoreBoost and the convKeys properties
can be readily defined through the template. Consider the following
example:
