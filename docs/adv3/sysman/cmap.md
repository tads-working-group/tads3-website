...
TADS](local.htm) \> Creating a Character-Mapping File  
[*Prev:* Translating Error Messages](errtrans.htm)    
...
leading ampersand (&) as it is in HTML source code. In the second form,
you use a number (the "0x" prefix indicates that the number is in
hexadecimal; you can specify a decimal number by omitting the "0x"
prefix, but the Unicode listings specify everything in hex so you would
probably want to use hex most of the time).
