...
[Table of Contents](toc.htm) \| [TADS 3 In Depth](depth.htm) \>
[Creating Dynamic Characters](t3actor.htm) \> Choosing a Conversation
System  
[*Prev:* Creating Dynamic Characters](t3actor.htm)     [*Next:*
Programming Conversations with NPCs](t3conv.htm)    
...
## Survey of IF conversation systems
...
^(*queries*)) possible threadings, where *topics* is the number of
possible topics (i.e., indirect objects of ASK ABOUT), and *queries* is
the number of ASK commands in an entire conversation. Menu authors are
faced with the exact same arithmetic, but *topics* is replaced by *menu
options*, which can easily be kept at three or four without arousing any
discomfort from the player; this would be impractical for ASK/TELL,
because limiting the player to three or four valid ASK topics at a time
would create an unacceptable "guess the noun" game. In practice, the
expanse of the menu tree is further reduced - vastly so - by looping
(where a branch leads back to a prior node) and converging (where
several branches lead to the same node).
...
The second item is a bit more subtle. There's a corollary to the
