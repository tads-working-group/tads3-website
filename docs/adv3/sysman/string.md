...
Intrinsics](builtins.htm){.nav} \> String\
[[*Prev:* StackFrameDesc](framedesc.htm){.nav}     [*Next:*
StringBuffer](strbuf.htm){.nav}     ]{.navnp}
:::

::: main
# String

\"String\" is a native TADS 3 datatype, but it\'s also an intrinsic
class. Any string value is an instance of the String class, so you can
call methods defined by the class on any string value.

## Value semantics

Strings have \"value semantics\". This means that a given string
value\'s text is immutable: once you\'ve created a string, the text
within that string never changes. All of the methods and operators that
might appear to change the value of a string actually create a new
string with the modified value, leaving the original value intact. For
...
[x]{.code}. In fact, the original string is not changed - if we display
the value of [y]{.code}, we\'ll see that [y]{.code} still contains
[\'foo\']{.code}. When the the last line above is executed, it creates a
new string to hold the concatenated value, and assigns the result to
[x]{.code}.
...
The addition operator [+]{.code} can be used to concatenate two strings
yielding a string that consists of the contents of the right-hand string
appended to the end of the contents of the left-hand string.
...
You can use the [+=]{.code} operator to replace the left-hand variable
...
[compareIgnoreCase(*str*)]{.code}

::: fdef
Compares this string to the second string *str*, ignoring differences in
upper/lower case. For example, \"A\" and \"a\" are treated as equal.
Returns an integer less than 0 if this string sorts before *str*, 0 if
the two strings are identical, or greater than 0 if this string sorts
after *str*.
...
::: fdef
Returns [true]{.code} if this string ends with *str*, [nil]{.code} if
not. This string ends with *str* if this string is at least as long as
*str*, and the last [str.length()]{.code} characters of this string are
the same as the characters of *str*.
:::

[]{#find}

[find(*target*, *index*?)]{.code}
...
1). If the target isn\'t found, the method returns [nil]{.code}. If
*target* is a regular expression, the method has the side effect of
...
it\'s a string, [find()]{.code} simply searches for the literal text of
the string. If *target* is a RexPattern, the method searches for a match
...
string for the purposes of \"\^\" and other special operators is still
the start of the overall string, regardless of the *index* value.
...
*target* can be a string or a [RexPattern](rexpat.htm) object. If it\'s
a string, the function searches for exact matches to the string. If
it\'s a pattern, the function searches for matches to the regular
expression.
...
*index*, *group1*\...)]{.code}, where *match* is a string with the text
of the match, *index* is the character index in the subject string of
the start of the match, and *group1* is the text of the first
parenthesized group in the regular expression, if there is one.
Arguments are added for any additional parenthesized groups. All of the
arguments are optional; the method calls *func* with as many arguments
as its actually expects, so if you don\'t need to know the index or
group values, you can simply omit the parameter variable for them. If
*func* requires more group parameters than there are actual matching
groups, the method passes [nil]{.code} values for the extra group
parameters.
...
[nil]{.code}. If *target* is a regular expression, the method has the
side effect of updating the [rexGroup()](tadsgen.htm#rexGroup) registers
...
(or, equivalently, [self.length()]{.code}) means that you want to search
...
For regular expression targets, the [\<FirstBegin\>]{.code} and
[\<FirstEnd\>]{.code} modes (see [regular expressions](regex.htm)) work
in mirror image compared to ordinary forward searches. The easiest way
to think about this is to picture the reverse search as a forward search
viewed in a mirror. For a reverse search, [\<FirstBegin\>]{.code} means
that the match with its *right* end closest to the starting point is
selected as the winner, while [\<FirstEnd\>]{.code} means that the match
with its *left* end closest to the starting point is selected. Since the
search proceeds right to left, closer to the starting point means
...
[findReplace(*oldStr*, *newStr*, *flags*?, *index*?, *limit*?)]{.code}

::: fdef
Finds one or more instances of the substring *oldStr* within the subject
string (i.e., [self]{.code}), and replaces them with the new substring
*newStr*.
...
If *oldStr* is a RexPattern, [%]{.code} characters in *newStr* have the
same special meaning they have in the replacement text in the
[rexReplace()](tadsgen.htm#rexReplace) function: [%1]{.code} is replaced
by the subject string text that matched the first parenthesized group in
the regular expression, [%2]{.code} by the text matching the second
group, and so on; [%\*]{.code} is replaced by the text matching the
entire regular expression pattern; and [%%]{.code} is replaced by a
single percent sign. When *oldStr* is a string value, [%]{.code}
characters in *newStr* have no special meaning and are simply used
literally as replacement text.
...
[ReplaceAll]{.code}, to replace all occurrences of the search string
within the subject string. If provided, *flags* is a bitwise combination
(with the [\|]{.code} operator) of the following flag values:

-   [ReplaceOnce]{.code}: replace only the first occurrence of the
    search string. This is ignored if *limit* is specified.
-   [ReplaceAll]{.code}: replace all occurrences of the search string.
    This is ignored if *limit* is specified. [ReplaceAll]{.code} is the
    default if [ReplaceOnce]{.code} isn\'t specified, and it supersedes
    [ReplaceOnce]{.code} if both are specified.
-   [ReplaceIgnoreCase]{.code}: ignore case (that is, capitalization)
    when searching for *oldStr*. By default, the search is
    case-sensitive, so capitals can only match capitals and minuscules
    can only match minuscules.
-   [ReplaceFollowCase]{.code}: each time a match is replaced, change
    ...
    capitals, all letters in the replacement text are capitalized; if
    all of the letters in the match are lower-case, the replacement text
    isn\'t changed; if the match has a mix of capitals and lower-case
    letters, the first lower-case letter in the replacement text is
    capitalized, and the rest are left unchanged.
-   [ReplaceSerial]{.code}: use the serial replacement mode. See
    [below](#findReplaceLists) for details.
...
last, and so on. Note that a negative index doesn\'t change the
direction of the search; it still runs left-to-right.
...
**[]{#findReplaceLists}Search lists:** Instead of searching for just a
single search string, you can search for several strings at once, by
using a list as the *oldStr* argument. This will search for each of the
items in the list, and replace each one with the *newStr* replacement
string.
...
Here\'s an example that replaces each of the special HTML characters
with their markup codes, using a single [findReplace()]{.code} call:
...
[findReplace()]{.code} starts by searching for all of the search terms
at once. It then replaces the single leftmost match with its
corresponding replacement text. (If two of the search strings match at
the same position, the one at the lower *oldStr* index takes
precedence.) If [ReplaceOnce]{.code} was specified, we\'re done.
Otherwise, [findReplace()]{.code} next repeats the search in the
*remainder* of the string, after (to the right of) that first
...
[ReplaceSerial]{.code} in the flags. In this mode,
[findReplace()]{.code} starts by searching *only* for the first *oldStr*
element. It replaces every match for the first term, or just the first
match if [ReplaceOnce]{.code} is specified. If a match was found and
[ReplaceOnce]{.code} was specified, we\'re done. Otherwise, we start
over with the *updated* string - containing all replacements for the
first term - and search this new string for the *second* search term. We
once again replace all occurrences of this term. We repeat this process
for each additional term in the *oldStr* list.
...
[]{#findReplaceFuncs}**Replacement function:** The replacement value
*newStr* is normally given as a literal string to substitute for each
occurrence of the search string. For more flexibility, though, you can
instead provide a function, which [findReplace()]{.code} calls for each
match to determine the replacement text for that match. This allows you
to vary the replacement text according to the exact text of the match
(which can vary if you\'re using the [ReplaceIgnoreCase]{.code} flag or
you\'re using a list of search strings), the position of the match
within the subject string, or whatever other conditions you choose.
...
function, because [findReplace]{.code} will only supply as many
...
can alternatively return [nil]{.code}, which is treated as an empty
string).
...
By default, this method scans the string for the characters [&]{.code}
(ampersand) and [\<]{.code} (less than), and converts these characters
to the sequences [&amp;]{.code} and [&lt;]{.code} respectively. This
conversion ensures that, when the string is rendered in HTML mode, the
display shows ampersands and less-than signs where they appeared in the
original string\'s text. In addition, you can specify a combination
(using the bitwise OR operator, [\|]{.code}) of the following flags to
perform additional conversions:

-   [HtmlifyTranslateSpaces]{.code}: converts each space character after
    the first space in a run of multiple spaces to the sequence
    [&nbsp;]{.code} (the HTML non-breaking space character). This ensure
    that a run of multiple spaces will be rendered visually with the
    same number of spaces as were in the original string. Without this
    flag, a run of spaces will appear visually as a single space when
    rendered as HTML, because HTML renderers combine adjacent spaces in
    HTML source (each run of multiple spaces is displayed as a single
    space). The method leaves the first space in a run of whitespace as
    an ordinary space character so that the HTML renderer can still
    break the line for word-wrapping purposes at the space.
-   [HtmlifyTranslateTabs]{.code}: converts each tab character
    ([\\t]{.code}) in the string to the sequence [\<tab\>]{.code}.
-   [HtmlifyTranslateNewlines]{.code}: converts each newline character
    ([\\n]{.code}) in the string to the sequence [\<br\>]{.code}.
-   [HtmlifyTranslateWhitespace]{.code}: this is simply a combination of
    [HtmlifyTranslateSpaces]{.code}, [HtmlifyTranslateTabs]{.code}, and
    [HtmlifyTranslateNewlines]{.code}.

This method is useful if you obtain a string from an external source,
such as from the user (via the [inputLine()]{.code} function, for
example) or from a text file, and you then want to display the string in
HTML mode. Without conversions, any markup-significant characters in the
string might not be displayed properly, since the HTML parser would
attempt to interpret the characters as HTML formatting codes. You can
use this method to ensure that a string obtained externally is displayed
verbatim in HTML mode.
:::
...
[mapToByteArray(*charset*?)]{.code}

::: fdef
Creates a [ByteArray](bytearr.h) object based on the contents of the
string. This is equivalent to calling [new ByteArray()]{.code} with this
string and character set as arguments.

If a *charset* value is included and isn\'t [nil]{.code}, it must be
either a [CharacterSet](charset.htm) object, or a string giving the name
of a character set. The method maps the string to bytes using the given
...
installation), an [UnknownCharSetException]{.code} is thrown. You can
determine whether the the character set is known using the
[isMappingKnown()](charset.htm#isMappingKnown) method on the
CharacterSet object.

If *charset* is omitted or [nil]{.code}, the method creates a ByteArray
with one byte per character of the string, using the Unicode character
code of each character as the byte value. Since a byte can only hold
values from 0 to 255, a numeric overflow error will be thrown if the
string contains any characters outside of this range.
:::

[]{#match}

[match(*target*, *index*?)]{.code}
...
If *index* is specified, it\'s an integer giving the starting position
to check for the match. 1 is the first character in the string; a
negative index is from the end of the string (-1 is the last character,
-2 is the second to last, and so on). If *index* is omitted, the method
checks for a match at the start of the string (as though *index* were
1).
...
subject string, you can end the pattern with \'\$\', since that will
allow the pattern to match only if the \'\$\' symbol lines up with the
end of the subject string after the match.)

If the string doesn\'t match *target*, the return value is [nil]{.code}.
Note that it\'s possible for a successful match to return 0: if *target*
is an empty string, that will match any subject with a length of zero;
and some regular expression patterns can match zero characters
successfully (e.g., \'a\*\', which matches zero or more \'a\'
characters).
...
[specialsToHtml(*stateObject*?)]{.code}

::: fdef
Returns a new string with any special TADS formatting characters in the
\"self\" string translated to their standard HTML equivalents. The
...
UI uses a standard browser as the user interface, though, so to write a
game for the Web UI, you must either avoid the TADS formatting codes and
use only standard HTML, or translate strings containing the TADS codes
into standard HTML. The former option would require a lot of work for an
...
writing a series of strings to the display, it\'s important keep track
of the global context across strings. You should use a separate object
for each window or output stream.

If you omit the state object or pass [nil]{.code}, the function will
...
after clearing the window), call its [resetState()]{.code} method.

The function performs the following translations:

-   [\\n]{.code} is translated to [\<BR\>]{.code} when it occurs within
    a line of text, or nothing if it occurs at the start of a new line
    (ensuring that it ends a line, but doesn\'t produce any blank
    lines).
-   [\\b]{.code} is translated to [\<BR\>]{.code} if it occurs at the
    start of a new line, or [\<BR\>\<BR\>]{.code} if it occurs within a
    line of text (ensuring that it always produces one blank line).
-   [\\\^]{.code} is removed, but causes the next non-markup character
    to be converted to upper-case.
-   [\\v]{.code} is removed, but causes the next non-markup character to
    be converted to lower-case.
-   [\\]{.code}  (quoted space) is converted to [&nbsp;]{.code} if it\'s
    followed by another quoted space, or to a regular space if followed
    by anything else. This reproduces the standard TADS quoted space
    behavior: a quoted space doesn\'t combine with adjacent quoted
    spaces, but does allow a line break. Any regular space characters
    adjacent to the quoted space in the source string are removed.
-   [\\t]{.code} is converted to a series of [&nbsp;]{.code} characters
    followed by one regular space, sufficient to pad the current line to
    the next multiple of four characters in length. This is a very rough
    approximation of the way [\\t]{.code} works in the console UI, but
    note how the algorithm merely counts *characters*, and doesn\'t take
    into account font metrics.
    (Because this doesn\'t take into account font metrics, it\'s mostly
    only useful with monospaced fonts. But if you\'re using the Web UI,
    you have access to the much greater capabilities of full HTML
    layout, so you shouldn\'t have much use for tabs anyway.)
-   [\<Q\>]{.code}\...[\</Q\>]{.code} tag sequences are converted to
    [&ldquo;\...&rdquo;]{.code} and [&lsquo;\...&rsquo;]{.code}
    sequences, alternating at each nesting level, with double quotes at
    the outermost level.
-   [\<BR HEIGHT=N\>]{.code} tags are converted to a series of *N*
    [\<BR\>]{.code} tags when used at the start of a line, or *N*+1
    [\<BR\>]{.code} tags when used within a line of text.
-   [\<P\>]{.code}, [\<DIV\>]{.code}, [\<CENTER\>]{.code},
    [\<TABLE\>]{.code}, [\<TD\>]{.code}, [\<TH\>]{.code}, and
    [\<CAPTION\>]{.code} tags (both open and close tags) are left
    exactly as given, but are recognized as line breaks for the purposes
    of translating [\\n]{.code}, [\\b]{.code}, and [\<BR
    HEIGHT=N\>]{.code}.

The *stateObject* value is for internal use within the function, and you
shouldn\'t have to access its properties directly. For reference,
though, they are:

-   [flags\_]{.code} is an integer containing a number of bit fields:
    -   0x0001 - on (non-zero) if the stream is in the midst of a line
    ...
    -   0x0010 - on if within a double-quoted attribute value in an HTML
        tag
    -   0x0020 - on if within a single-quoted attribute value in an HTML
        tag
    -   0x0040 - on if the last character was an ordinary space
    -   0x0080 - on if the last character was a quoted space (\'\\ \')
    -   0x0100 - on if the \<Q\> tag quote nesting level is odd, off if
        the level is even (at even levels, double quotes are used; at
        odd levels, single quotes are used)
    -   0x0200 - on if an HTML entity (& sequence) is in progress (only
        used with specialsToText())
    -   0x3000 - the current tab stop column (shift this value right 12
        bits to get the integer value: that is, compute [((obj.flags\_ &
        0x3000) \>\> 12)]{.code}. This is the number of characters in
        the line since the last multiple of 4 columns.
-   [tag\_]{.code} is a string containing the text of the tag in
    progress. When a string ends in mid-tag, this contains the fragment
    of the tag up to the end of the string, so that the next call can
    resume parsing the tag where the last call left off.
:::

[]{#specialsToText}

[specialsToText(*stateObject*?)]{.code}

::: fdef
Returns a new string with any special TADS formatting characters in the
\"self\" string translated to their plain text equivalents. This
function is similar to specialsToHtml(), but converts the string to
plain text rather than HTML. The conversion is designed to replicate the
effect of displaying the string through the TADS console output
...
-   [\\n]{.code} is translated to [\\n]{.code} when it occurs within a
    line of text, or nothing if it occurs at the start of a new line
    (ensuring that it ends a line, but doesn\'t produce any blank
    lines).
-   [\\b]{.code} is translated to [\\n]{.code} if it occurs at the start
...
-   [\\v]{.code} is removed, but causes the next non-markup character to
    be converted to lower-case.
-   [\\]{.code}  (quoted space) is converted to a regular space.
-   [\<Q\>]{.code}\...[\</Q\>]{.code} tag sequences are converted to
    [\"\...\"]{.code} and [\'\...\']{.code} sequences, alternating at
    each nesting level, with double quotes at the outermost level.
-   [\<BR HEIGHT=N\>]{.code} tags are converted to a series of *N*
    [\\n]{.code} characters when used at the start of a line, or *N*+1
    [\\n]{.code} characters when used within a line of text.
-   [\<P\>]{.code} is converted to [\\n]{.code} if it appears at the
    start of a line, or [\\n\\n]{.code} within a line.
-   [\<DIV\>]{.code}, [\<CENTER\>]{.code}, [\<TABLE\>]{.code},
    [\<TD\>]{.code}, [\<TH\>]{.code}, and [\<CAPTION\>]{.code} tags
    (both open and close tags) are converted to [\\n]{.code}.
-   [\<Tag\>]{.code} for any tag not mentioned above is simply stripped
    out.
-   [&nbsp;]{.code} is converted to a space.
-   [&gt;]{.code} is converted to \>.
-   [&lt;]{.code} is converted to \<.
-   [&amp;]{.code} is converted to &.
-   [&quot;]{.code}, [&ldquo;]{.code}, and [&rdquo;]{.code} are
    converted to \" (a plain double-quote).
-   [&lsquo;]{.code} and [&rsquo;]{.code} are converted to \' (a plain
    single-quote).
-   [&dddd;]{.code} (where the *d*s are digits) is converted to the
    Unicode character with value *dddd*.
:::

[]{#splice}

[splice(*index*, *deleteLength*, *insertString*?)]{.code}
...
substrings of the original string, but [splice()]{.code} is more concise
...
-   A string. [split]{.code} searches for exact matches to this
    substring within the subject string, and splits the string at each
    instance.
-   A [RexPattern](rexpat.htm) object. The method searches for matches
    to the regular expression, and splits the string at each match
    found.
-   An integer, which must be at least 1. The method splits the string
    into substrings of exactly this length (except that the last
    substring in the list might be shorter, since it\'ll have
    whatever\'s left over at the end).

If *delim* is omitted or [nil]{.code}, the default is 1 (i.e., split the
...
entire original string. If *limit* is omitted or [nil]{.code}, the
method splits the string at every instance of the delimiter without a
limit.
...
[\'one,two,three\'.split(\',\')]{.code} returns [\[\'one\', \'two\',
\'three\'\]]{.code}\
[\'one,two, three, four\'.split(R\',\<space\>\*\')]{.code} returns
[\[\'one\', \'two\', \'three\', \'four\'\]]{.code}\
[\'one,two,three\'.split(\',\', 2)]{.code} returns [\[\'one\',
\'two,three\'\]]{.code}\
[\'abcdefghi\'.split(2)]{.code} returns [\[\'ab\', \'cd\', \'ef\',
\'gh\', \'i\'\]]{.code}
:::
...
::: fdef
Returns true if this string starts with *str*, [nil]{.code} if not. This
string starts with *str* if this string is at least as long as *str*,
and the first [str.length()]{.code} characters match the characters of
*str*.
:::

[substr(*start*, *length*?)]{.code}
...
upper case as \'SS\', so its case folding is lower case \'ss\'. This
allows the strings \'WEISS\' and \'weiß\' to match when case-folded, for
example. There are a number of other ligatures and accented characters
that undergo similar expansions when case-folded.
:::
...
back into a string using the [makeString()]{.code} function in the
...
[\'%C3%A1\'.urlDecode()]{.code} returns [\'á\']{.code}. Any %xx sequence
that doesn\'t form a valid UTF-8 character is converted to \'?\'.
:::
...
encoded as \"%2C\". Characters outside of the plain ASCII range (\\u0000
to \\u007F) are encoded using the multi-byte UTF-8 representation. For
...
would encode all of the scheme and path characters (such as the \":\"
and \"//\" in the \"http://\" prefix), which would make the string
unusable as a URL. This method is intended only for encoding the
building blocks of URL strings, such as the value portion of a
