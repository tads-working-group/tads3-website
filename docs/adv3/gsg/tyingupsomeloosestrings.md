...
a string constant, so that the definition is effectively equivalent to:\
...
tags occur in succession), whereas the former, \\b, may result in
several blank lines, depending on what is printed next. If you *want*
several blank lines, then you need to use \\b.\
\
